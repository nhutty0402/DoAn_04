...
        return
      }
const TripDetailsPage = ({ trip, router, activeTab, setActiveTab, resolvedParams }) => {
  // Hàm trợ giúp để lấy thông tin badge, bạn có thể tùy chỉnh
  const getStatusBadge = (status) => {
    if (status === 'Sắp diễn ra') return { variant: "default", label: "Sắp diễn ra" };
    if (status === 'Đang diễn ra') return { variant: "destructive", label: "Đang diễn ra" };
    return { variant: "secondary", label: "Đã kết thúc" };
  };
...
   <main className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4 mb-6">
          {/* Nút Back và Tiêu đề */}
          <div className="flex items-center gap-2 flex-1 mb-4 sm:mb-0">
          ...
          <div className="flex items-center gap-2 self-start sm:self-center">
            <Badge {...getStatusBadge(trip.trang_thai)}>
              {getStatusBadge(trip.trang_thai).label}
            </Badge>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-muted-foreground">Xuất phát</p>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        {/* Thay thế các dòng dưới bằng component thật của bạn */}
        <TabsContent value="overview">
          <div>Nội dung tab Tổng quan</div>
        </TabsContent>
        <TabsContent value="itinerary">
          <div>Nội dung tab Lịch trình</div>
        </TabsContent>
        <TabsContent value="members">
          <div>Nội dung tab Thành viên</div>
        </TabsContent>
        <TabsContent value="expenses">
          <div>Nội dung tab Chi phí</div>
        </TabsContent>
        <TabsContent value="chat">
          <div>Nội dung tab Chat</div>
        </TabsContent>
        <TabsContent value="maps">
          <div>Nội dung tab Bản đồ</div>
        </TabsContent>
        <TabsContent value="settings">
          <div>Nội dung tab Cài đặt</div>
        </TabsContent>
      </Tabs>
    </main>
    </div>
