...
                {trip.ten_chuyen_di}
              </h1>
              <p className="text-muted-foreground mt-1 font-[family-name:var(--font-dm-sans)]">{trip.mo_ta}</p>
            </div>
                        <div className="flex items-center gap-3">
                            {/* Avatars stack */}
                            <div className="flex -space-x-2">
                                {(trip && (trip as any).members ? (trip as any).members.slice(0, 4) : []).map((m: any) => (
                                    <Avatar key={m.id} className="h-8 w-8 ring-1 ring-background">
                                        <AvatarImage src={`https://ui-avatars.com/api/?name=${encodeURIComponent(m.name || "?")}`} alt={m.name} />
...
        {/* Tabs */}
                        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                            <div className="overflow-x-auto">
                                <TabsList className="inline-grid w-max grid-cols-7 gap-2 px-2">
...
          </TabsContent>
                    ...
                    <TabsContent value="members">
                        <MembersTab members={(trip as any).members || []} tripId={resolvedParams.id} />
                    </TabsContent>
