...
export default function PublicTripDetailPage() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const el = document.getElementById("trip-pdf-content") || document.body
      ...
      if (imgPdfHeight <= pdfHeight) {
        ...
        // If content taller than one page, split vertically
        const pageCanvasHeight = Math.floor((imgHeight * pdfHeight) / imgPdfHeight)
        let position = 0
        while (position < imgHeight) {
          const sliceCanvas = document.createElement("canvas")
          sliceCanvas.width = canvas.width
          sliceCanvas.height = Math.min(pageCanvasHeight, canvas.height - position)
          ...
          pdf.addImage(sliceData, "JPEG", 0, 0, pdfWidth, (sliceCanvas.height * pdfWidth) / sliceCanvas.width)
          position += sliceCanvas.height
        }
      }

      const filename = `${trip.title.replace(/[^a-z0-9_-]/gi, "_")}.pdf`
      pdf.save(filename)
    } catch (err) {
      // If html2canvas/jsPDF not installed or any error, fallback/helpful message
      // eslint-disable-next-line no-console
      console.error("PDF generation failed:", err)
      // show simple fallback: ask user to install packages or use print
      // Keep UX friendly: offer to call window.print()
      const installHint =
        "Không thể tạo PDF tự động. Hãy cài đặt " +
        "`html2canvas` và `jspdf` (npm i html2canvas jspdf) hoặc thử in trang bằng trình duyệt."
      // eslint-disable-next-line no-alert
      if (confirm(installHint + "\n\nBạn muốn in trang hiện tại thay thế?")) {
        window.print()
      }
    }
  }
  ...
  return (
    <div id="trip-pdf-content" className="min-h-screen bg-gradient-to-br from-sky-50 to-blue-50">
      {/* Header */}
