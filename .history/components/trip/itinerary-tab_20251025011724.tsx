...
export function ItineraryTab({ tripId }: ItineraryTabProps) {
  ...
  useEffect(() => {
    const fetchDaysList = async () => {
      ...
      try {
        // Gọi API với header Authorization và query param
        ...
        const mappedDays = daysFromApi.map((day: any) => ({
          ...
          ngay: day.ngay,
          tieu_De: day.tieu_de || day.tieuDe,
  ...
  const checkTimeOverlap = (dayId: string, newPoi: Poi) => {
  ...
  const handleDeletePoi = (dayId: string, poiId: string) => {
    setDays(days.map((day: Day) => (
      day.id === dayId 
        ? { ...day, pois: day.pois.filter((poi: Poi) => poi.id !== poiId) } 
        : day
    )))
    toast({
      ...
      {isLoadingDays ? (
        <div className="flex items-center justify-center py-8">
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-sm text-muted-foreground">Đang tải lịch trình...</p>
          </div>
        </div>
      ) : (
        <>
                  ...
                  <motion.div layout className="p-4 border rounded-lg bg-white shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex flex-col">
                        <h3 className="text-lg font-medium">{day.tieu_De}</h3>
                    ...
                    <div className="space-y-4">
                      {day.pois.map((poi: Poi, index: number) => (
                        <Card key={poi.id}>
                          <CardHeader>
                            <div className="flex items-start justify-between">
                              <CardTitle className="text-base">{poi.tenDiaDiem}</CardTitle>
                              <Badge className={`${getPoiTypeLabel(poi.loaiDiaDiem).color} ml-2`}>
                          ...
                          <CardContent className="text-sm space-y-2">
                            ...
                            {poi.ghiChu && <p className="text-muted-foreground">{poi.ghiChu}</p>}
                          </CardContent>
                        </Card>
                      ))}
                    </div>
