...
interface ApiResponse {
  status: string;
  data: ApiItineraryDay[];
}

interface ItineraryTabProps {
  tripId: string | number;
  ghiChu?: string;
  pois: any[]; // Sẽ thêm interface POI sau
}
...
export function ItineraryTab({ tripId }: { tripId: string }) {
  ...
  useEffect(() => {
    const fetchItinerary = async () => {
      try {
        ...
        const transformedData: ItineraryDay[] = response.data.data.map(day => ({
          id: day.lich_trinh_ngay_id.toString(),
          ngay: day.ngay,
          tieuDe: day.tieu_de,
          ghiChu: day.ghi_chu,
          pois: [] // Will need to fetch POIs separately or handle them differently
        }))
  ...
  const handleDeletePoi = (dayId: string, poiId: string) => {
    // ... (hàm này không thay đổi) ...
    setDays(days.map((day: ItineraryDay) => (day.id === dayId ? { ...day, pois: day.pois.filter((poi: POI) => poi.id !== poiId) } : day)))
    toast({
