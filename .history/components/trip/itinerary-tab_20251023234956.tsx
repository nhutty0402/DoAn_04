...
// <-- THÊM INTERFACE ĐỂ SỬA LỖI TYPESCRIPT -->
interface Poi {
  id: string
  tenDiaDiem: string
  loaiDiaDiem: string
...
interface Day {
  ...
  tieuDe: string
  ghiChu: string
  pois: Poi[] // <-- Sử dụng interface Poi ở đây
}
...
export function ItineraryTab({ tripId }: ItineraryTabProps) {
  // <-- SỬA: Dùng interface Day[] thay vì any[] -->
  ...
  useEffect(() => {
   ...
   try {
      ...
      const response = await axios.get(
        `https://travel-planner-imdw.onrender.com/api/lich-trinh-ngay/chuyen-di/${tripId}`, // <-- SỬA Ở ĐÂY
        {
      ...
      // ...
        ...
        const fetchedDays: Day[] = response.data.map((dayFromApi: any) => ({
          ...
          pois: (dayFromApi.pois || []).map((poiFromApi: any): Poi => ({
            ...
            toaDo: poiFromApi.toa_do || { lat: 0, lng: 0 }, // Đảm bảo toaDo luôn tồn tại
  ...
  // <-- SỬA: Thêm kiểu cho newPoi (dữ liệu thô từ modal) -->
  // Đây là hàm bạn báo lỗi
  const checkTimeOverlap = (
    dayId: string,
    newPoiData: { id?: string; gioBatDau: string; gioKetThuc: string }
  ) => {
    const day = days.find((d) => d.id === dayId)
    if (!day) return [] // Nếu không tìm thấy ngày, trả về mảng rỗng

    const newStart = new Date(`2024-01-01 ${newPoiData.gioBatDau}`)
    const newEnd = new Date(`2024-01-01 ${newPoiData.gioKetThuc}`)

    // Giờ đây, 'poi' sẽ được TypeScript hiểu là kiểu 'Poi' (vì 'day.pois' là 'Poi[]')
    // Sẽ không còn lỗi đỏ ở 'poi.gioBatDau' hay 'poi.gioKetThuc'
    return day.pois.filter((poi) => {
      if (poi.id === newPoiData.id) return false // Bỏ qua chính nó (khi edit)
  ...
  // <-- SỬA: Thêm kiểu cho poiData (dữ liệu thô từ modal) -->
  const handleAddPoi = (dayId: string, poiData: Omit<Poi, 'id' | 'thoiGianDiChuyen'>) => {
    // Giờ checkTimeOverlap sẽ nhận đúng kiểu
    ...
    const newPoi: Poi = {
      ...
      // Đảm bảo có toaDo (nếu modal không trả về)
      toaDo: poiData.toaDo || { lat: 0, lng: 0 } 
    }
