...
import { useState, useEffect } from "react" // <-- SỬA
...
// Xóa mockItinerary, chúng ta sẽ tải từ API
...
export function ItineraryTab({ tripId }: ItineraryTabProps) {
  const [days, setDays] = useState<any[]>([]) // <-- SỬA: Bắt đầu với mảng rỗng
  const [isLoadingDays, setIsLoadingDays] = useState(true) // <-- THÊM: State loading
  ...
  // <-- THÊM: useEffect để tải dữ liệu lịch trình -->
  useEffect(() => {
    const fetchItineraryDays = async () => {
      ...
      try {
        // Gọi API GET với query param 'chuyen_di_id'
        const response = await axios.get(
          "https://travel-planner-imdw.onrender.com/api/lich-trinh-ngay",
          {
            params: {
              chuyen_di_id: tripId, // Lọc theo tripId
            },
            headers: {
        ...
        // Giả sử API trả về luôn cả các POI lồng nhau (pois)
        const fetchedDays = response.data.map((dayFromApi: any) => ({
          id: dayFromApi.id, // Giả sử API trả về 'id' (giống handleAddDay)
          ngay: dayFromApi.ngay,
          tieuDe: dayFromApi.tieu_de,
          ghiChu: dayFromApi.ghi_chu,
          // Giả sử POIs được trả về lồng nhau và cũng cần map
          pois: (dayFromApi.pois || []).map((poiFromApi: any) => ({
            id: poiFromApi.id, // Giả sử API trả về 'id'
            ...
            toaDo: poiFromApi.toa_do,
            thoiGianDiChuyen: poiFromApi.thoi_gian_di_chuyen || 0,
          })).sort((a: any, b: any) => a.gioBatDau.localeCompare(b.gioBatDau)), // Sắp xếp POI theo giờ
        }))
    ...
    if (tripId) {
      ...
      setIsLoadingDays(false) // Không có tripId, không cần tải
    }
  ...
  const checkTimeOverlap = (dayId: string, newPoi: any) => {
    ...
    return day.pois.filter((pois) => {
      if (pois.id === newPoi.id) return false
      const poiStart = new Date(`2024-01-01 ${pois.gioBatDau}`)
      const poiEnd = new Date(`2024-01-01 ${pois.gioKetThuc}`)
      return newStart < poiEnd && newEnd > poiStart
    })
  }

  // (hàm handleAddDay không thay đổi so với code bạn cung cấp)
  ...
  // (hàm handleAddPoi không thay đổi)
  ...
  // (hàm handleDeletePoi không thay đổi)
  ...
  // (hàm handleReorderDays không thay đổi)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Timeline */}
      {/* <-- SỬA: Chỉ hiển thị khi KHÔNG loading VÀ có ngày --> */}
      ...
      {/* Trạng thái trống */}
      {/* <-- SỬA: Chỉ hiển thị khi KHÔNG loading VÀ KHÔNG có ngày --> */}
      {!isLoadingDays && days.length === 0 && (
      ...
      {/* Modals */}
      {/* (Phần Modals không thay đổi) */}
      {showAddDayModal && (
        <AddDayModal
          onClose={() => setShowAddDayModal(false)}
          onSubmit={handleAddDay}
          // isLoading={isAddingDay} // Bạn đã xóa dòng này, tôi giữ nguyên
        />
      )}
