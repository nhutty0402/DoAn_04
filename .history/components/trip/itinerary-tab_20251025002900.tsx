  ...
  useEffect(() => {
    const fetchDaysList = async () => {
      ...
      try {
        ...
        // SỬA: Sử dụng query param thay vì path param để filter theo chuyen_di_id
        // Giả sử API GET /api/lich-trinh-ngay/ hỗ trợ query ?chuyen_di_id=...
        ...
        // SỬA: Filter thêm theo chuyen_di_id nếu API trả về tất cả (fallback, nếu query param không filter server-side)
        ...
        const mappedDays = daysFromApi.map((day: any) => ({
          id: day.lich_trinh_ngay_id || day.id, // Sử dụng snake_case từ API
          ngay: day.ngay,
        ...
        // Fallback: Sử dụng mock data nếu API lỗi (tùy chọn, có thể xóa nếu không cần)
  ...
  const handleAddDay = async (dayData: any) => {
    ...
    try {
      ...
      const newDayForState = {
        id: newDayFromApi.lich_trinh_ngay_id || newDayFromApi.id, // Ưu tiên snake_case
        ngay: newDayFromApi.ngay,
        tieuDe: newDayFromApi.tieu_de || newDayFromApi.tieuDe || "",
        ghiChu: newDayFromApi.ghi_chu || newDayFromApi.ghiChu || "",
    ...
    <div className="space-y-6">
      ...
      {showAddDayModal && (
        <AddDayModal
          ...
          // SỬA: Xóa prop isLoading để tránh crash (modal chưa hỗ trợ)
          // Nếu cần loading state trong modal, cập nhật AddDayModal để nhận và sử dụng prop này (ví dụ: disabled button khi isLoading)
        />
      )}
