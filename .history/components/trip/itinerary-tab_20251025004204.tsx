...
// Mock itinerary data (giữ nguyên, chỉ dùng cho fallback nếu cần - nhưng tạm thời không dùng cho 404)
const mockItinerary = [
  {
    id: "day1",
    ngay: "2024-03-15",
  ...
  useEffect(() => {
    const fetchDaysList = async () => {
      ...
      try {
        ...
        // Gọi API không có query param (theo spec: GET /api/lich-trinh-ngay/)
        ...
        // Filter theo chuyen_di_id (client-side)
        ...
        // SỬA: Xử lý 404 cụ thể - coi như chưa có dữ liệu, không dùng mock
        if (error.response?.status === 404) {
          console.warn("API endpoint 404 - Endpoint chưa được triển khai hoặc không tồn tại. Hiển thị empty state.")
          toast({
            title: "Chưa có dữ liệu",
            description: "Endpoint API chưa sẵn sàng hoặc chưa có ngày nào cho chuyến đi này. Bạn có thể thêm ngày mới.",
            variant: "default", // Không destructive, chỉ thông báo
          })
          setDays([]) // Empty state để UI hiển thị "Chưa có lịch trình"
          return
        }

        // Fallback khác: Chỉ dùng mock nếu lỗi không phải 404/401 (ví dụ network error)
        console.warn("API lỗi khác, fallback sang mock data tạm thời")
        setDays(mockItinerary)

        toast({
          title: "Lỗi khi tải danh sách",
          description: "Không thể tải danh sách ngày từ server. Đang sử dụng dữ liệu mẫu tạm thời.",
          variant: "destructive",
        })
      } finally {
  ...
  const handleAddDay = async (dayData: any) => {
    ...
    try {
      ...
      const newDayForState = {
        id: newDayFromApi.lich_trinh_ngay_id || newDayFromApi.id,
      ...
      if (axios.isAxiosError(error) && error.response?.status === 401) {
        ...
        toast({
          title: "Lỗi",
          description: "Không thể thêm ngày mới. Vui lòng thử lại. (Kiểm tra endpoint POST /api/lich-trinh-ngay)",
          variant: "destructive",
        })
      }
    } finally {
