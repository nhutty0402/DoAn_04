...
export function ItineraryTab({ tripId }: ItineraryTabProps) {
  ...
  // Helper to get token and redirect to login if missing
  ...
  useEffect(() => {
    const fetchDays = async () => {
      ...
      try {
        ...
        // Try path-based endpoint first (some backends use /api/lich-trinh-ngay/{tripId})
        let res
        try {
          res = await axios.get(`https://travel-planner-imdw.onrender.com/api/lich-trinh-ngay/${tripId}`, {
            headers: { Authorization: `Bearer ${token}` },
          })
        } catch (err: any) {
          // If 404, try the query-param variant
          if (axios.isAxiosError(err) && err.response?.status === 404) {
            res = await axios.get("https://travel-planner-imdw.onrender.com/api/lich-trinh-ngay/", {
              headers: { Authorization: `Bearer ${token}` },
              params: { chuyen_di_id: tripId },
            })
          } else {
            throw err
          }
        ...
        // Expected response: array of objects with keys: lich_trinh_ngay_id, chuyen_di_id, ngay, tieu_de, ghi_chu
        const apiData = Array.isArray(res.data) ? res.data : res.data.data || res.data

        const transformed = apiData.map((d: any) => ({
          id: String(d.lich_trinh_ngay_id ?? d.id ?? Math.random()),
          ngay: d.ngay,
          tieuDe: d.tieu_de ?? d.tieuDe,
          ghiChu: d.ghi_chu ?? d.ghiChu ?? "",
          pois: d.pois ?? [],
        }))

        setDays(transformed)
      } catch (err: any) {
        console.error("Failed to fetch itinerary days:", err)
  ...
  const checkTimeOverlap = (dayId: string, newPoi: any) => {
    ...
    return day.pois.filter((poi: POI) => {
  ...
  const handleAddDay = async (dayData: any) => {
    ...
    // Lấy token và redirect nếu không có
    const token = getTokenOrRedirect()
    if (!token) {
  ...
  const handleDeletePoi = (dayId: string, poiId: string) => {
    // ... (hàm này không thay đổi) ...
    setDays(days.map((day: any) => (day.id === dayId ? { ...day, pois: day.pois.filter((poi: POI) => poi.id !== poiId) } : day)))
    toast({
                              ...
                              <div className="flex items-start justify-between">
                                <div className="flex-1">
                                  ...
                                  <div className="flex items-center gap-4 text-sm text-muted-foreground mb-2">
                                    ...
                                    {typeof poi.thoiGianDiChuyen === 'number' && poi.thoiGianDiChuyen > 0 && (
