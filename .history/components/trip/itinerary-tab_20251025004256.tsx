...
interface PoiType {
  id: string
  tenDiaDiem: string
  loaiDiaDiem: "landmark" | "restaurant" | "hotel" | "shopping" | "beach" | "historic"
  gioBatDau: string
  gioKetThuc: string
  ghiChu?: string
...
export function ItineraryTab({ tripId, isOwner = false }: ItineraryTabProps) {
  // State
  ...
  // Helper function to get POI type label and color
  const getPoiTypeLabel = (type: keyof typeof poiTypeInfo) => {
    return poiTypeInfo[type] || poiTypeInfo.landmark
  }

  // Helper function to check time overlaps
  const checkTimeOverlap = (dayId: string, newPoi: PoiType) => {
    ...
    return day.pois.filter((poi: PoiType) => {
  ...
  const handleAddDay = async (formData: any) => {
    ...
    try {
      ...
      if (error.response?.status === 401 || error.response?.status === 403 || error.message === "No token found") {
        toast({
      ...
      toast({
        title: "Error",
        description: "Could not add day. Please try again.",
        variant: "destructive",
      })
    } finally {
  ...
  // Handler for adding a POI to a day
  const handleAddPoi = async (dayId: string, poiData: any) => {
    try {
      ...
      if (overlaps.length > 0) {
        toast({
          title: "Warning",
          description: "Time slot is already occupied. Please choose a different time.",
          variant: "destructive",
        })
        return
      }
      ...
      const response = await axios.post(
        `https://travel-planner-imdw.onrender.com/api/poi`,
        {
          ...poiData,
      ...
      toast({
        title: "Success",
        description: "New point of interest added",
      })
    } catch (error: any) {
      ...
      toast({
        title: "Error",
        description: "Could not add point of interest. Please try again.",
        variant: "destructive",
      })
    }
  }
  ...
  const handleAddPoi = async (dayId: string, poiData: any) => {
    try {
      ...
      if (overlaps.length > 0) {
        toast({
          title: "Cảnh báo",
          description: "Thời gian này đã có địa điểm khác. Vui lòng chọn thời gian khác.",
          variant: "destructive",
        })
        return
      }
      ...
      toast({
        title: "Lỗi",
        description: "Không thể thêm địa điểm. Vui lòng thử lại sau.",
        variant: "destructive",
      })
    }
  }

  // Helper function for POI type labels
  ...
  // Helper function to check time overlaps when adding POIs
  // Helper functions
  const checkTimeOverlap = (dayId: string, newPoi: any) => {
    const day = days.find((d) => d.id === dayId)
    if (!day) return []
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {!isLoadingDays && days.length === 0 && (
        <div className="text-center">
          <h3 className="mb-4 text-lg font-medium">Chưa có ngày nào trong lịch trình</h3>
              ...
              <motion.div layout className="p-4 border rounded-lg bg-white shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex flex-col">
                    <h3 className="text-lg font-medium">{day.tieuDe}</h3>
                      ...
                      <Card key={poi.id}>
                        <CardHeader>
                          <div className="flex items-start justify-between">
                            ...
                            <Badge
                              className={`${
                                getPoiTypeLabel(poi.loaiDiaDiem).color
                              } ml-2`}
                            >
                        ...
                        <CardContent className="text-sm space-y-2">
                          ...
                          {poi.ghiChu && (
                            <p className="text-muted-foreground">{poi.ghiChu}</p>
                          )}
                        </CardContent>
                      </Card>
                    ))}
