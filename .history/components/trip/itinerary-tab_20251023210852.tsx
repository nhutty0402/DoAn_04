...
export function ItineraryTab({ tripId }: ItineraryTabProps) {
  ...
  const [isAddingDay, setIsAddingDay] = useState(false) // <-- 2. Thêm state loading
  ...
  // <-- 3. Cập nhật hàm handleAddDay -->
  const handleAddDay = async (dayData: any) => {
    setIsAddingDay(true) // Bắt đầu loading
    try {
      // Chuẩn bị payload cho API
      // Ánh xạ 'tieuDe' -> 'tieu_de' và 'ghiChu' -> 'ghi_chu'
      const payload = {
        chuyen_di_id: tripId, // Lấy từ props
        ngay: dayData.ngay,
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Modals */}
      {/* <-- 4. Cập nhật gọi Modal để truyền state loading --> */}
      {/* {showAddDayModal && (
        <AddDayModal
          onClose={() => setShowAddDayModal(false)}
          onSubmit={handleAddDay}
          isLoading={isAddingDay} // Truyền state loading cho modal
        />
      )}
