  ...
  useEffect(() => {
    const fetchDaysList = async () => {
      ...
      try {
        ...
        // Thử URL với path param thay vì query param
        ...
        const mappedDays = daysFromApi.map((day: any) => ({
          ...
          pois: day.pois || [], // We'll handle POIs in a separate API call if needed
        }))
      ...
      <div className="bg-yellow-50 p-4 rounded-lg mb-4">
        ...
        <pre className="text-sm whitespace-pre-wrap">
          {JSON.stringify({
            ...
            token: Cookies.get("token")?.slice(0, 10) + "..."
          }, null, 2)}
        </pre>
      </div>
      ...
      <div className="space-y-4">
        {isLoadingDays ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">Chưa có lịch trình</h3>
            ...
            {days.map((day, index) => (
              <Card key={day.id} className="border-l-4 border-l-primary">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      <CardTitle>{day.tieuDe || 'Không có tiêu đề'}</CardTitle>
                      ...
                      {day.ghiChu && (
                        <p className="text-sm text-muted-foreground mt-1">{day.ghiChu}</p>
                      )}
                    </div>
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" size="sm" onClick={() => setShowAddPoiModal(day.id)}>
