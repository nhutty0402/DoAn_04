...
// Mock itinerary data (giữ nguyên)
const mockItinerary = [
  {
    id: "day1",
    ngay: "2024-03-15",
...
  		  </h2>
  		</div>
  		<Button onClick={() => setShowAddDayModal(true)} className="bg-primary hover:bg-primary/90" disabled={isAddingDay}>
...
  							<CardTitle className="font-[family-name:var(--font-space-grotesk)] truncate">{day.tieuDe}</CardTitle>
...
  								{poiIndex < day.pois.length - 1 && <div className="w-0.5 h-full min-h-[2rem] bg-border my-2" />}
  							  </div>
...
  		  isSubmitting={isAddingDay} // <-- Sửa lại tên prop (giả sử) và truyền state
  		/>
  	  )}
...
  		  isSubmitting={isSubmittingPoi} // <-- Thêm prop loading
  		/>
  	  )}
...
  		  isSubmitting={isEditingDay} // <-- Thêm prop loading
  		/>
  	  )}
    </div>
  )
