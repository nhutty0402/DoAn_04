...
// Mock itinerary data (giữ nguyên, chỉ dùng cho fallback nếu cần)
const mockItinerary = [
  {
    id: "day1",
    ngay: "2024-03-15",
...
export function ItineraryTab({ tripId }: ItineraryTabProps) {
  ...
  useEffect(() => {
    const fetchDaysList = async () => {
      ...
      try {
        // Log tripId để debug
        console.log("Fetching days for tripId:", tripId)

        // SỬA: Gọi API không có query param (theo spec bạn cung cấp: GET /api/lich-trinh-ngay/)
        // Server sẽ trả về tất cả ngày, client filter theo chuyen_di_id
        ...
        if (Array.isArray(response.data)) {
          ...
          daysFromApi = [response.data] // Nếu là một object duy nhất, wrap trong array
        }

        // Filter theo chuyen_di_id (client-side, vì API không hỗ trợ query param)
        daysFromApi = daysFromApi.filter((day: any) => day.chuyen_di_id === parseInt(tripId))

        console.log("Processed days from API (filtered):", daysFromApi)

        const mappedDays = daysFromApi.map((day: any) => ({
          id: day.lich_trinh_ngay_id || day.id, // ĐÚNG: Sử dụng lich_trinh_ngay_id từ API
          ngay: day.ngay, // ĐÚNG: Trực tiếp từ API
          tieuDe: day.tieu_de || day.tieuDe || "", // ĐÚNG: Ưu tiên tieu_de (snake_case từ API)
          ghiChu: day.ghi_chu || day.ghiChu || "", // ĐÚNG: Ưu tiên ghi_chu (snake_case từ API)
          pois: day.pois || [], // Hiện tại chưa có POI từ API, sẽ handle riêng sau
        }))
        ...
        if (error.response?.status === 404) {
          console.warn("API endpoint 404 - Có thể chưa có dữ liệu hoặc endpoint sai. Fallback mock data.")
          toast({
            title: "Lỗi khi tải danh sách",
            description: "Không tìm thấy dữ liệu. Đang sử dụng dữ liệu mẫu.",
            variant: "destructive",
          })
          setDays([]) // Không dùng mock nếu muốn empty
          return
        }

        // Fallback: Sử dụng mock data nếu API lỗi khác (tùy chọn, có thể xóa nếu không cần)
        console.warn("API lỗi, fallback sang mock data")
        setDays(mockItinerary)

        toast({
          title: "Lỗi khi tải danh sách",
          description: "Không thể tải danh sách ngày. Đang sử dụng dữ liệu mẫu.",
          variant: "destructive",
        })
      } finally {
  ...
  // SỬA: Cập nhật handleAddDay để consistent với API structure (sử dụng lich_trinh_ngay_id)
  const handleAddDay = async (dayData: any) => {
    ...
    try {
      const payload = {
        chuyen_di_id: tripId,
        ngay: dayData.ngay,
        tieu_de: dayData.tieuDe, // ĐÚNG: snake_case cho API
        ghi_chu: dayData.ghiChu, // ĐÚNG: snake_case cho API
      }
      ...
      // SỬA: Ánh xạ với lich_trinh_ngay_id từ API response
      const newDayForState = {
        id: newDayFromApi.lich_trinh_ngay_id || newDayFromApi.id, // ĐÚNG: Ưu tiên lich_trinh_ngay_id
        ngay: newDayFromApi.ngay, // ĐÚNG
        tieuDe: newDayFromApi.tieu_de || newDayFromApi.tieuDe || "", // ĐÚNG
        ghiChu: newDayFromApi.ghi_chu || newDayFromApi.ghiChu || "", // ĐÚNG
    ...
    <div className="space-y-6">
      ...
      {showAddDayModal && (
        <AddDayModal
          ...
          // Không truyền isLoading để tránh crash
        />
      )}
