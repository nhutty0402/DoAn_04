...
export function ItineraryTab({ tripId }: ItineraryTabProps) {
  ...
  useEffect(() => {
    const fetchDays = async () => {
      ...
      try {
        // Try to fetch days for this tripId (primary URL)
        const base = "https://travel-planner-imdw.onrender.com/api"
        const urlPrimary = `${base}/lich-trinh-ngay/?chuyen_di_id=${encodeURIComponent(tripId)}`

        let res
        try {
          res = await axios.get(urlPrimary, { headers: { Authorization: `Bearer ${token}` } })
        } catch (err: any) {
          // If 404, try alternate endpoint naming (underscore) as some APIs use different conventions
          if (err?.response?.status === 404) {
            const urlAlt = `${base}/lich_trinh_ngay/?chuyen_di_id=${encodeURIComponent(tripId)}`
            console.warn(`Primary endpoint returned 404, trying alternate URL: ${urlAlt}`)
            res = await axios.get(urlAlt, { headers: { Authorization: `Bearer ${token}` } })
          } else {
            throw err
          }
        }
        ...
        console.debug("Itinerary fetch status:", res.status, res.statusText)
        console.debug("Itinerary fetch data:", res.data)
        ...
        console.error("Failed to fetch itinerary days:", error?.response ?? error)
        // If server returned a response, include status in toast for easier debugging
        const status = error?.response?.status
        toast({
          title: "Lỗi khi tải lịch trình",
          description: status
            ? `Không thể tải danh sách ngày (status ${status}). Kiểm tra endpoint hoặc token.`
            : "Không thể tải danh sách ngày. Vui lòng thử lại sau.",
          variant: "destructive",
        })
        // Do not silently fallback to mock data when authentication is required — keep list empty
        setDays([])
      } finally {
        setIsLoadingDays(false)
      }
        ...
        <Reorder.Group axis="y" values={days} onReorder={handleReorderDays}>
          {days.map((day, dayIndex) => (
            <Reorder.Item key={day.id} value={day}>
              ...
              >
              {/* hiện danh sách thêm ngày chuyến đi */}
