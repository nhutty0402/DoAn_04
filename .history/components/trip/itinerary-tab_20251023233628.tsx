...
// Mock itinerary data
// Đây là một hàm async (bất đồng bộ) để gọi API
async function layDanhSachLichTrinh() {
  const apiUrl = 'https://travel-planner-imdw.onrender.com/api/lich-trinh-ngay/';
  ...
  try {
    ...
    const response = await fetch(apiUrl, {
      method: 'GET', // GET là phương thức mặc định, nhưng viết rõ sẽ dễ hiểu hơn
      headers: {
        'Content-Type': 'application/json',
        // Thêm các headers khác nếu API của bạn yêu cầu (ví dụ: Authorization token)
      },
    });

    // Kiểm tra xem API có trả về lỗi hay không (ví dụ: 404, 500)
    if (!response.ok) {
    ...
    // Tại đây, 'data' sẽ là một mảng (Array) chứa các đối tượng lịch trình
    // mỗi đối tượng sẽ có các trường như:
    // {
    //   "lich_trinh_ngay_id": ...,
    //   "chuyen_di_id": ...,
    //   "ngay": "...",
    ...
    // }
    //
    // Bạn có thể dùng dữ liệu này để cập nhật state trong ứng dụng của mình
    // ví dụ: setItineraryData(data);

    return data; // Trả về dữ liệu nếu bạn muốn dùng ở nơi khác

  } catch (error) {
    // Bắt và xử lý bất kỳ lỗi nào xảy ra trong quá trình gọi API (ví dụ: lỗi mạng)
