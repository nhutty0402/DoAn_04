...
// <-- 1. CẬP NHẬT PROPS: Thêm ngày bắt đầu/kết thúc của chuyến đi -->
interface ItineraryTabProps {
  tripId: string
  ngayBatDau: string // Ví dụ: "2024-03-15" (Định dạng YYYY-MM-DD)
  ngayKetThuc: string // Ví dụ: "2024-03-20" (Định dạng YYYY-MM-DD)
}

// <-- 2. CẬP NHẬT COMPONENT: Nhận props mới -->
export function ItineraryTab({ tripId, ngayBatDau, ngayKetThuc }: ItineraryTabProps) {
  ...
  // <-- 3. CẬP NHẬT HÀM: Thêm logic validation ngày -->
  const handleAddDay = async (dayData: any) => {
    ...
    const newDateStr = dayData.ngay // Ngày mới người dùng chọn
    if (!newDateStr) {
      toast({ title: "Lỗi", description: "Vui lòng chọn một ngày.", variant: "destructive" })
      setIsAddingDay(false)
      return
    }

    // Chuyển sang Date object để so sánh (thêm T00:00:00 để tránh lỗi múi giờ)
    ...
    // 1. Kiểm tra có SỚM HƠN ngày bắt đầu chuyến đi?
    ...
    // 2. Kiểm tra có LỐ (MUỘN HƠN) ngày kết thúc chuyến đi?
    ...
    // 3. Kiểm tra có TRÙNG với ngày đã có trong lịch trình?
    // So sánh chuỗi 'YYYY-MM-DD' là đủ
    ...
    try {
      ...
      // <-- 4. BẮT LỖI 409 (CONFLICT - TRÙNG LẶP) TỪ SERVER -->
      if (axios.isAxiosError(error) && error.response?.status === 409) {
        toast({
          title: "Lỗi trùng lặp từ máy chủ",
    ...
    <div className="space-y-6">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className="text-sm text-muted-foreground">
                              {new Date(day.ngay + "T00:00:00").toLocaleDateString("vi-VN", {
                                // Thêm T00:00:00 để tránh lỗi hiển thị sai ngày do múi giờ
                                weekday: "long",
                                year: "numeric",
                                month: "long",
                                day: "numeric",
      ...
      {showAddDayModal && (
        <AddDayModal
          ...
          // Tạm thời vẫn xóa prop isLoading để tránh lỗi crash component
          // Bạn nên cập nhật AddDayModal để nhận prop 'isLoading'
        />
      )}
      ...
      {editingDay && (
        <EditDayModal
          ...
          onSubmit={(dayData) => {
            // Bạn cũng nên thêm validation cho việc Sửa ngày ở đây
