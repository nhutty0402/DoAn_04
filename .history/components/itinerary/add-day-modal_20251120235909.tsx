...
export function AddDayModal({ onClose, onSubmit }: AddDayModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Call onSubmit (may be sync or async). If it returns a promise, await it.
      const result = onSubmit(formData)
      if (result && typeof (result as any).then === "function") {
        await result
      }

      // Refresh the current route so newly added day is reloaded
      router.refresh()

      toast({
        title: "Thành công",
        description: "Đã thêm ngày mới",
      })
