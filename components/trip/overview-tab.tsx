...
export function OverviewTab({ trip }: OverviewTabProps) {
  ...
  const stats = [
    {
      ...
      description: "15/03 - 20/03/2024",
    },
    {
      title: "Thành viên",
      value: `${trip.soThanhVien} người`,
      icon: <Users className="h-5 w-5 text-primary" />,
      description: "3 đã xác nhận, 1 chờ phản hồi",
    },
    {
      title: "Điểm đến",
      value: "12 địa điểm",
      icon: <MapPin className="h-5 w-5 text-primary" />,
      description: "Đà Nẵng, Hội An, Huế",
    },
    {
      title: "Tổng chi phí",
      value: "15.5M VNĐ",
      icon: <DollarSign className="h-5 w-5 text-primary" />,
      description: "3.9M VNĐ/người",
    },
  ]
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: trip.tenChuyenDi,
          text: `Tham gia chuyến đi ${trip.tenChuyenDi} cùng tôi!`,
  ...
  const handleSharePublic = async () => {
    const publicLink = `${window.location.origin}/feed/${trip.id}`
    try {
      if (navigator.share) {
        await navigator.share({
          title: trip.tenChuyenDi,
          text: `Khám phá chuyến đi tuyệt vời: ${trip.tenChuyenDi}`,
          url: publicLink,
        })
      } else {
