...
import { useState, useEffect, useRef } from "react" //PHẦN DI CHUYỂN CHUỘT useRef
...
// PHẦN DI CHUYỂN CHUỘT TỰ ĐỘNG
const emailLoginRef   = useRef<HTMLInputElement>(null)
const emailForgotRef  = useRef<HTMLInputElement>(null)
const otpRef          = useRef<HTMLInputElement>(null)

// Tự động focus khi mở form hoặc chuyển tab
useEffect(() => {
  const timer = setTimeout(() => {
    if (formMode === "login" && emailLoginRef.current) {
      emailLoginRef.current.focus()
    } else if (formMode === "forgotPassword" && emailForgotRef.current) {
      emailForgotRef.current.focus()
    } else if (formMode === "verifyOtp" && otpRef.current) {
...
// ---------------------------------------------------------
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("VN-Travel_user", JSON.stringify(data))
      ...
      if (inviteCode && data.token) {
        try {
          ...
          toast({
            ...
            description: `Chào mừng bạn trở lại VN-Travel`,
          })
          ...
          <form onSubmit={handleForgotPassword} className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="relative">
                <Mail className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                <Input
                ref={emailForgotRef}      // Thêm dòng này
                  id="email"
                  type="email"
                  placeholder="your@email.com"
                  value={email}
